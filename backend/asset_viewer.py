...
def _extract_generation_from_comfy_prompt(prompt_graph: Any) -> dict[str, Any]:
	...
	positive_prompt = _extract_text_from_ref(prompt_graph, inputs.get("positive") if isinstance(inputs, dict) else None)
	if positive_prompt:
		generation["positive_prompt"] = positive_prompt

	negative_prompt = _extract_text_from_ref(prompt_graph, inputs.get("negative") if isinstance(inputs, dict) else None)
	if negative_prompt:
